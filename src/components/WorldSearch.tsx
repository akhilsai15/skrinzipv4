...
import { Search, X, Flame, ChevronDown, Check, Clock } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { useWorlds } from "../hooks/useWorldMembership";
import { StealthAvatar } from "./StealthAvatar";
...
const MOCK_MEMBERS = [
  {
    id: "u1",
    username: "Rahul_Gamer",
    level: "pioneer",
    community: "SkrimGamers",
  },
  {
    id: "u2",
    username: "GamingGuru47",
    level: "explorer",
    community: "GameZone",
  },
...
export function WorldSearch({
  ...
  useEffect(() => {
    if (isOpen) {
      const saved = localStorage.getItem("worlds_recent_searches");
  ...
  const handleSaveRecent = (q: string) => {
    if (!q.trim()) return;
    const newRecent = [q, ...recentSearches.filter((r) => r !== q)].slice(0, 5);
    setRecentSearches(newRecent);
    localStorage.setItem("worlds_recent_searches", JSON.stringify(newRecent));
  };

  const removeRecent = (e: React.MouseEvent, q: string) => {
    e.stopPropagation();
    const newRecent = recentSearches.filter((r) => r !== q);
    setRecentSearches(newRecent);
    localStorage.setItem("worlds_recent_searches", JSON.stringify(newRecent));
  };
  ...
  const clearAllRecent = () => {
    setRecentSearches([]);
    localStorage.removeItem("worlds_recent_searches");
  };

  const handleSearchTerm = (term: string) => {
    setQuery(term);
    handleSaveRecent(term);
  };
  ...
  const filteredWorlds = useMemo(() => {
    ...
    if (selectedCategories.length > 0) {
      result = result.filter(
        (w) => w.category && selectedCategories.includes(w.category),
      );
    }
    if (selectedSize === "Small (< 1k members)")
      result = result.filter((w) => w.members < 1000);
    if (selectedSize === "Medium (1k - 10k)")
      result = result.filter((w) => w.members >= 1000 && w.members <= 10000);
    if (selectedSize === "Large (10k+)")
      result = result.filter((w) => w.members > 10000);

    // Sort
    if (selectedSort === "Most members") {
  ...
  const filteredMembers = useMemo(() => {
    ...
    return MOCK_MEMBERS.filter((m) =>
      m.username.toLowerCase().includes(lowerQ),
    );
  }, [debouncedQuery]);
  ...
  const renderHighlighted = (text: string, highlight: string) => {
    ...
    return (
      <>
        {parts.map((part, i) =>
          regex.test(part) ? (
            <span key={i} className="text-[#7B2FF7]">
              {part}
            </span>
          ) : (
      ...
      <div className="fixed inset-0 z-50 flex flex-col pointer-events-none">
        ...
        >
          ...
          <div className="px-4 py-3 flex items-center gap-3">
            <div className="relative flex-1 group">
              ...
              <div className="relative bg-[#1a1a24] rounded-xl flex items-center px-4 h-12 shadow-lg">
                ...
                <input
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && handleSaveRecent(query)
                  }
            ...
            <button
              onClick={onClose}
              className="text-white hover:text-[#B026FF] font-medium text-sm px-2"
            >
              Cancel
            </button>
          </div>
          ...
          <div className="px-4 pb-3 flex items-center gap-2 overflow-x-auto hide-scrollbar whitespace-nowrap">
            ...
            >
              ...
              {activeAdvancedFilterCount > 0 && (
                <span className="ml-1 px-1.5 py-0.5 rounded text-[10px] bg-[#B026FF] text-white">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <button
                        ...
                        className="text-[#7B2FF7] text-[13px] font-bold hover:text-[#B026FF] mt-2 w-max"
                      >
                        Clear all
                      </button>
                    </div>
                ...
                {filteredWorlds.length === 0 && filteredMembers.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <h3 className="text-white text-lg font-bold mb-2">
                      {debouncedQuery
                        ? `No worlds found for "${debouncedQuery}"`
                        : "No worlds match your filters."}
                    </h3>
                    <p className="text-[#888899] text-sm mb-6 max-w-[250px]">
                      {debouncedQuery
                        ? "Try a different name or explore categories."
                          ...
                          {filteredWorlds.map((world, i) => (
                            ...
                            >
                              <div
                                ...
                                style={{
                                  background: `linear-gradient(to bottom right, ${getAtmosphereColor(world.atmosphere)}40, transparent)`,
                                }}
                              >
                                {world.initials}
                              </div>
                              <div className="flex-1">
                                ...
                                <div className="flex items-center gap-2 mt-0.5">
                                  <span className="text-[#888899] text-[12px]">
                                    {world.category}
                                  </span>
                                  <span className="text-[#888899] text-[12px]">
                                    •
                                  ...
                                  <span className="text-[#888899] text-[12px]">
                                    {(world.members / 1000).toFixed(1)}k members
                                  </span>
                                  {world.active && (
                              ...
                              <button
                                className={`px-4 py-1.5 rounded-full text-[12px] font-bold transition-all ${world.joined ? "bg-white/10 text-white" : "bg-[#7B2FF7] text-white hover:bg-[#B026FF]"}`}
                              >
                        ...
                        <div className="flex flex-col gap-3">
                          {filteredMembers.map((member, i) => (
                            <motion.div
                              ...
                              animate={{ opacity: 1, y: 0 }}
                              transition={{
                                delay: (i + filteredWorlds.length) * 0.03,
                              }}
                              key={member.id}
                              className="flex items-center p-3 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors cursor-pointer group"
                            >
                              ...
                              <div className="flex-1">
                                ...
                                <div className="flex items-center gap-1.5 mt-0.5">
                                  <span className="text-[10px] bg-[#9CA3AF]/20 text-[#9CA3AF] px-1.5 py-0.5 rounded-sm font-bold uppercase">
                                    {member.level}
                                  </span>
                                  <span className="text-[#888899] text-[12px]">
                                    in {member.community}
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between p-4 border-b border-white/5">
                ...
                <button
                  onClick={() => setShowMoreFilters(false)}
                  className="text-[#888899] hover:text-white p-2 bg-white/5 rounded-full"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 pb-24 space-y-8">
                {/* Category */}
                <div>
                  <h4 className="text-[12px] font-bold text-[#888899] uppercase tracking-wider mb-3">
                    Category
                  </h4>
                  <div className="flex flex-wrap gap-2">
                    {[
                      ...
                      const isSelected =
                        selectedCategories.includes(simpleName);
                      return (
                        <button
                          key={cat}
                          onClick={() => {
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-3">
                    {[
                      ...
                      >
                        <div
                          className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${selectedSize === size ? "border-[#7B2FF7]" : "border-[#888899] group-hover:border-white"}`}
                        >
                          {selectedSize === size && (
                            <div className="w-2.5 h-2.5 bg-[#7B2FF7] rounded-full" />
                          )}
                        </div>
                        <span
                          className={`text-[14px] ${selectedSize === size ? "text-white font-medium" : "text-[#888899] group-hover:text-white"}`}
                        >
                          {size}
                        </span>
                      </label>
                    ...
                    {["India", "Global"].map((loc) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-6 py-2 rounded-xl text-[13px] font-bold transition-all border ${isSelected ? "bg-[#7B2FF7]/20 border-[#7B2FF7] text-white" : "bg-white/5 border-white/10 text-white/70 hover:bg-white/10 hover:text-white"}`}
                        >
                          {loc}{" "}
                          {isSelected && (
                            <Check size={12} className="inline ml-1" />
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-3">
                    {[
                      ...
                      >
                        <div
                          className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${selectedSort === sort ? "border-[#7B2FF7]" : "border-[#888899] group-hover:border-white"}`}
                        >
                          {selectedSort === sort && (
                            <div className="w-2.5 h-2.5 bg-[#7B2FF7] rounded-full" />
                          )}
                        ...
                        <span
                          className={`text-[14px] ${selectedSort === sort ? "text-white font-medium" : "text-[#888899] group-hover:text-white"}`}
                        >
                          {sort}
                        </span>
                      </label>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 bg-[#111115] border-t border-white/5 flex items-center gap-3">
                <button
                  onClick={clearAdvancedFilters}
                  className="flex-1 py-3.5 rounded-xl bg-white/5 text-white font-bold hover:bg-white/10 transition-colors"
                >
                  Reset
                </button>
                <button
