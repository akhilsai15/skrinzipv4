...
export function ChatInput({ currentMood, onSetMood, onSendMessage, onSendVoice, onSendGif, onSendSticker, onSendAttachment, onOpenGamePicker, onTyping, replyingTo, onCancelReply }: Props) {
              ...
              {!isRecordingLocked ? (
                ...
                >
                  <motion.div 
                    ...
                    className="text-white/50 text-[11px] whitespace-nowrap flex items-center gap-1 cursor-ew-resize"
                  >
                     <span>← Cancel</span>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button onClick={() => stopRecording(true)} className="p-1.5 text-red-400 hover:text-red-300 transition-colors">
                    <Trash2 size={16} />
                  </button>
                  <button onClick={() => stopRecording(false)} className="w-7 h-7 bg-[#B026FF] rounded-full text-white shadow-[0_0_10px_rgba(176,38,255,0.6)] flex items-center justify-center">
            ...
            <button 
              ...
              onPointerMove={(e) => {
                if (isRecording && !isRecordingLocked) {
                  if (e.clientX < window.innerWidth / 2 || e.clientX < e.currentTarget.getBoundingClientRect().left - 100) {
                    stopRecording(true);
                  } else if (e.clientY < window.innerHeight - 150 || e.clientY < e.currentTarget.getBoundingClientRect().top - 80) {
                    setIsRecordingLocked(true);
                  }
                }
              }}
              ...
              style={{
                background: text.trim() ? (sendBtnBg || 'linear-gradient(to bottom right, #7B2FF7, #B026FF)') : '',
                boxShadow: text.trim() ? (sendBtnGlow || '0 0 15px rgba(176,38,255,0.4)') : '',
              }}
            >
