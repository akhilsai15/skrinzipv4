...
export default function WorldsScreen() {
  ...
  const [animationStage, setAnimationStage] = useState<number>(
    localStorage.getItem("skrimchat_worlds_seen") ? 4 : 0,
  );
  const [showInterests, setShowInterests] = useState(
    !localStorage.getItem("skrimchat_worlds_interests"),
  );
  ...
  if (animationStage < 4) {
    return <EntryAnimation stage={animationStage} />;
  }
      ...
      <main className="flex-1 flex flex-col gap-8 pb-10 relative z-10 mt-6">
        ...
        <AnimatePresence>
          {myWorlds.length > 0 ? (
            ...
            >
              <div className="px-4 flex items-center mb-4 sticky top-[64px] z-40 bg-[#0A0A14]/90 backdrop-blur-md py-2 -mt-2">
        ...
        <section className="px-4">
          <div className="flex items-center mb-4 sticky top-[64px] z-40 bg-[#0A0A14]/90 backdrop-blur-md py-2 -mt-2">
...
function EntryAnimation({ stage }: { stage: number }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#0A0A14] flex flex-col items-center justify-center overflow-hidden">
